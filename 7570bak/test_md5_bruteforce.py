...
TARGET_PASSWORD = "1" # Example target
...
# Actual Debug Output Buffer (for 1-digit kernel, size 10*18)
GPU_DEBUG_KEYSPACE = 10 # Size expected by the .cl file
DEBUG_ENTRY_SIZE = 1 + 1 + 16 
...
print("Executing GPU kernel (NOTE: Using 1-digit debug kernel from .cl file)...")
# Use keyspace size appropriate for the 1-digit kernel
global_size = (GPU_DEBUG_KEYSPACE,)
local_size = None

# Pass arguments appropriate for the 1-digit debug kernel
# Need to pass length=1 even though Python config might be different
gpu_password_len_arg = np.int32(1)
...
try:
    bruteforce_kernel.set_args(target_hash_buf,
                               charset_buf,
                               charset_len_arg,
                               gpu_password_len_arg, # Pass 1 for kernel
                               result_found_buf,    # Dummy
                               found_password_buf,  # Dummy
                               debug_output_buf)

    print(f"Launching {GPU_DEBUG_KEYSPACE} work items for 1-digit debug kernel...")
    start_time = time.time()
    ...
    print(f"GPU Hash Rate (1-digit kernel): {gpu_hashes_sec:,.2f} hashes/sec")
    ...
    for gid in range(GPU_DEBUG_KEYSPACE):
        ...
        gpu_match_result = int(debug_output_host[offset])
        gpu_candidate_char = chr(debug_output_host[offset + 1])
        gpu_hash_bytes = debug_output_host[offset + 2 : offset + 2 + 16].tobytes()
        py_hash_bytes = hashlib.md5(gpu_candidate_char.encode('ascii')).digest()
        ...
        print(f"{gid: <7} | '{gpu_candidate_char}'       | {expected_match_str: <27} | {gpu_match_str: <10} | {gpu_hash_bytes[:8].hex()}... ({match_status})")

    print("------------------------------------------------------------------------------------------")
    if overall_match:
        print("GPU Overall Result: PASSED - GPU comparison results match expected results for all 1-digit candidates.")
    else:
        print("GPU Overall Result: FAILED - GPU comparison results differ from expected for one or more 1-digit candidates.")
    ...
    print("\n" + "="*30)
    print("      Analysis Summary (1-Digit Debug)")
    print("="*30)
    print(f"Target Password: '{TARGET_PASSWORD}'")
